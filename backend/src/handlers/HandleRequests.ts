...
export const UpdateProfile = async ( req : Request , res : Response ) => {
    ...
    try {
        ...
        if( userWithCleanHP  &&  userWithCleanHP?.email !== req.user.email ) return res.status(409).json( { error:errorsMessagesArray.userAlreadyExists('Handle Profile Alias').message } ) ;
        ...
        return res.status(500).json( { error: errorsMessagesArray.updateError } )
...
export const UploadImage = async ( req : Request, res : Response ) => {
    ...
    try {
        ...
        formidable( {multiples: false} )?.parse( req , ( error, fields, files ) => {
            ...
            if (!files.file || !files.file[0]) return res.status(400).json({ error: errorsMessagesArray.notImageUploadAlready }) ;
            


            const validMimeTypes = ['image/jpeg', 'image/png', 'image/jpg']

            if(  !validMimeTypes.includes( files.file[0].mimetype || '' )  ) return res.status(400).json( {error: errorsMessagesArray.invalidImageFormat.message } ) ;
